...
// Robust CSV row parser to handle quoted text
...
export default function Home() {
  ...
  const ann: FrameAnnotation = useMemo(() => {
    ...
    return annotations[video.id]?.[frame.name] ?? DEFAULT_ANN;
  }, [annotations, video, frame]);

  /* ── read image file → blob URL ────────────────────────────────── */
  const getFrameUrl = useCallback(
    async (v: VideoEntry, frameData: FrameData): Promise<string> => {
      const key = `${v.id}/${frameData.name}`;
      const cached = blobCache.current.get(key);
      if (cached) return cached;
      try {
        const file = await frameData.handle.getFile();
  ...
  const loadDataset = useCallback(
    async (dirHandle: FileSystemDirectoryHandle) => {
      try {
        ...
        // Find the CSV (looks in root folder first)
        for await (const entry of dirHandle.values()) {
          if (entry.kind === "file" && entry.name.endsWith(".csv")) {
            csvHandle = entry as FileSystemFileHandle;
            break;
          }
        }

        if (!csvHandle) {
          throw new Error("Could not find any .csv file in the selected folder.");
        }

        // Recursively find all image files to match them easily regardless of their directory nesting
        const fileHandles = new Map<string, FileSystemFileHandle>();
        async function collectImages(dir: FileSystemDirectoryHandle) {
          for await (const entry of dir.values()) {
            if (entry.kind === "file" && IMAGE_EXT.has(ext(entry.name))) {
               fileHandles.set(entry.name, entry as FileSystemFileHandle);
            } else if (entry.kind === "directory") {
               await collectImages(entry as FileSystemDirectoryHandle);
            }
          }
        }
        await collectImages(dirHandle);
        ...
        const requiredHeaders = ["video_id", "frame_id", "pred_C1", "pred_C2", "pred_C3", "image_path"];
        for (const rh of requiredHeaders) {
          if (!(rh in hIdx)) throw new Error(`Missing required CSV column: ${rh}`);
        }

        const parsedAnnotations: Annotations = {};
        const vidToFrames: Record<string, { num: number; frameData: FrameData }[]> = {};

        for (let i = 1; i < lines.length; i++) {
          const row = parseCSVRow(lines[i].trim());
          if (row.length < 6) continue;

          const vid = row[hIdx["video_id"]];
          const frameNumStr = row[hIdx["frame_id"]];
          const imgPath = row[hIdx["image_path"]];
          
          // Extract filename from the path
          const imgName = imgPath.split("/").pop() || "";
          const handle = fileHandles.get(imgName);
          
          if (!handle) continue; // Skip if we couldn't find the physical image file

          if (!vidToFrames[vid]) vidToFrames[vid] = [];
          vidToFrames[vid].push({ 
            num: parseInt(frameNumStr, 10), 
            frameData: { name: imgName, handle } 
          });
          ...
          parsedAnnotations[vid][imgName] = {
            pred: [
              parseFloat(row[hIdx["pred_C1"]]) || 0,
              parseFloat(row[hIdx["pred_C2"]]) || 0,
              parseFloat(row[hIdx["pred_C3"]]) || 0,
            ]
          };
        }
        ...
        const vids: VideoEntry[] = Object.keys(vidToFrames).map((vid) => {
          ...
          return {
            id: vid,
            name: `Video ${vid.substring(0, 15)}...`, // Trim long hashes
            frames: sorted,
          };
        });

        // Optional: sort alphabetically or numerically
        vids.sort((a, b) => a.id.localeCompare(b.id));
  ...
  const browseFolder = useCallback(async () => {
    ...
    try {
      ...
      if (vids.length === 0) {
        setError("No valid frames matched between the CSV and the directory contents.");
        setBrowsing(false);
        return;
      }
  ...
  /* ── per-video stats (Based on Predictions) ────────────────────── */
  const stats = useMemo(
    () =>
      videos.map((v) => {
        ...
        for (const f of v.frames) {
          const a = va[f.name];
          if (a) {
            if (a.pred[0] > 0) counts[0]++;
            if (a.pred[1] > 0) counts[1]++;
            if (a.pred[2] > 0) counts[2]++;
          }
        }
        return { counts };
      }),
      ...
      <div className="h-screen flex flex-col items-center justify-center gap-10" style={{ background: "var(--bg)", color: "var(--fg)" }}>
        ...
        <div className="flex flex-col items-center gap-1.5">
          ...
          <p className="text-[11px] uppercase tracking-[0.3em]" style={{ color: "var(--fg-muted)" }}>
            Review Single-Annotator Labels
          </p>
        </div>
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[11px] mt-1" style={{ color: "var(--fg-faint)" }}>
            Select root dataset folder containing your CSV and images
          </p>
        </div>
          ...
          <div className="flex-1 flex items-center justify-center min-h-0 relative" style={{ background: "var(--bg-deep)" }}>
            ...
            >
              <div className="flex items-center gap-3">
                <span className="text-[10px] font-bold w-8 tracking-wide" style={{ color: "var(--fg)" }}>
                  PRED
                </span>
                <div className="flex gap-2">
                  {CRITERIA.map((c, ci) => {
                    const val = ann.pred[ci];
                      ...
                      <div key={ci} className="w-8 flex justify-center items-center">
                        <div
                          ...
                          style={{
                            ...
                            boxShadow: isActive ? `0 0 8px ${rgba(c.color, 0.4)}` : "none",
                          }}
                          title={`${c.label}: ${val}`}
                        >
                          {c.label}
          ...
          <div className="h-5 shrink-0 flex items-center justify-center px-3" style={{ background: "var(--scrubber-bg)" }}>
            ...
            >
              {video?.frames.map((f, i) => {
                const fa = annotations[video.id]?.[f.name] ?? DEFAULT_ANN;
                const cur = i === frameIdx;
                const hasAny = fa.pred[0] > 0 || fa.pred[1] > 0 || fa.pred[2] > 0;
                return (
                  <div
                    key={i}
                  ...
                  >
                    {CRITERIA.map((c, ci) => (
                      <div
                        ...
                        className="flex-1"
                        style={{
                          background: fa.pred[ci] > 0 ? c.color : "var(--scrubber-empty)",
                        }}
                      />
                    ))}
                  </div>
